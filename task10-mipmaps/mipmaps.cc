...
void loadMipmapTexture(std::vector<std::string> const &filenames,
                       unsigned int *id) {
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAX_LEVEL, filenames.size() - 1);
    ...
    for (unsigned int i = 0; i < filenames.size(); i++) {
        ...
        if (data) {
            ...
            std::cout << "Failed to load texture: " << filenames[i]
                      << std::endl;
        }
...
void renderLoop(Settings &setup) {
    ...
    loadMipmapTexture({"map01.jpg", "map02.jpg", "map03.jpg", "map04.jpg",
                       "map05.jpg", "map06.jpg"},
                      &texture_id);
