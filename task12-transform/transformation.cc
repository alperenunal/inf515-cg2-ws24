...
struct Settings {
    std::string const title = "INF515-Transformation";
...
// ------------------------------------------------------------------------ //
double getDeltaValueA(int idx) {
    double time = glfwGetTime();
    // --- version I
    double interval = 4.0;
    time += idx * 0.10;
    double dy = (time - ((int)(time / interval)) * interval) / interval;
    if (dy > 0.5) {
        dy = 1.0 - dy;
    }
    dy *= 2.00;
...
// ------------------------------------------------------------------------ //
double getDeltaValueB(int idx, int count) {
    double time = glfwGetTime();
    // --- version II
    double dy = std::sin(2.0 * time + idx * (2.0 * M_PI / count));
    dy /= 4.0;
    dy += 0.75;
    return dy;
}
...
// ------------------------------------------------------------------------ //
void updateTransformation(int program_ID) {
    // ToDo - step 1: enter your code for transformation here
    // - draw the objects repeatedly in a loop
    // - scale and transform the instances to arrange them in a pattern

    int count = 32;
    glm::vec3 z = glm::vec3(0.0f, 0.0f, 1.0f);

    for (int i = 0; i < count; i++) {
        float dy = getDeltaValueB(i, count);

        glm::mat4 transform = glm::mat4(1.0f);
        transform = glm::rotate(transform, glm::radians(360.0f / count * i), z);
        transform =
            glm::translate(transform, glm::vec3(0.0f, dy * 0.75f, 0.0f));
        transform = glm::scale(transform, glm::vec3(0.25f, 0.25f, 1.0f));

        unsigned int loc = glGetUniformLocation(program_ID, "transform");
